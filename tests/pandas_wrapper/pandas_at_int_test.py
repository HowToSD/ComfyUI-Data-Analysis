...
from pandas_wrapper.pandas_at_int import PandasAtInt

class TestPandasAtInt(unittest.TestCase):
    def setUp(self):
        """Set up test data."""
        self.node = PandasAtInt()
        self.test_cases = [
            # Test with integer values and string indices
            (
                pd.DataFrame({"a": [123]}, index=["row1"]),
                "row1", "string", "a", "string",
                (123,)
            ),
            (
                pd.DataFrame({"a": [789]}, index=["row2"]),
            ...
            (
                pd.DataFrame({"a": [456]}, index=[0]),
                "0", "int", "a", "string",
                (456,)
            ),
            (
                pd.DataFrame({"a": [987]}, index=[1]),
                "1", "int", "a", "string",
                (987,)
            ),
            ...
            (
                pd.DataFrame({1: [321]}, index=[0]),
                "0", "int", 1, "int",
                (321,)
            ),
            (
                pd.DataFrame({2: [654]}, index=[1]),
                "1", "int", 2, "int",
                (654,)
            ),
            ...
            (
                pd.DataFrame({1: [111]}, index=["event"]),
                "event", "string", 1, "int",
                (111,)
            ),
        ]
